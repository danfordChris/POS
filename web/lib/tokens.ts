// Duka Stock — neumorphic design tokens (TS mirror of web/app/tokens.css / the
// canonical docs/design/interfaces/design_handoff_neumorphic_system/tokens.json).
// Use CSS custom properties (`var(--surface)`, `shadow-elev-md`) in components;
// reach for these objects only where a JS value is unavoidable (canvas, charts,
// inline style computed at runtime).
...
export const fontFamily = "'Nunito', ui-sans-serif, system-ui, sans-serif";

type Elev = { x: number; y: number; blur: number };

export function raisedShadow(mode: ThemeMode, e: Elev): string {
  const t = colors[mode];
  return `${e.x}px ${e.y}px ${e.blur}px ${t.shadowDark}, -${e.x}px -${e.y}px ${e.blur}px ${t.shadowLight}`;
}

export function insetShadow(mode: ThemeMode, e: Elev): string {
  const t = colors[mode];
